...
Assesses only **objective, routine data the clinician already has — never subjective patient-reported history.** There is no "previous adverse drug reaction" or "previous treatment failure" input anywhere in this pipeline: those are real clinical signals, but they aren't the kind of routine, verifiable data this tier is scoped to reason about. The inputs are exactly:

| Parameter | Widget | Values |
|-----------|--------|--------|
| `age` | Number input | Years |
| `weight` | Number input | kg |
| `lft` | Selectbox | Normal / Abnormal / Unknown |
| `rft_egfr` | Selectbox | Normal / Abnormal / Unknown |
| `cbc_platelets` | Selectbox | Normal / Abnormal / Unknown |
| `pt_inr_aptt` | Selectbox, or a numeric INR value | Normal / Abnormal / Unknown / a raw number |

- **`assess_renal_function(rft_egfr)`** — flags nephrotoxicity risk per **KDIGO 2024**. Because the input is a coarse category rather than a raw eGFR number, an "Abnormal" result is reported as exactly that — a flag naming KDIGO as the guideline for further staging — rather than a fabricated numeric G-stage the engine has no data to support.
- **`assess_hepatic_function(lft)`** — flags hepatic-impairment risk for empirical dose-caution framing per **CDSCO's National Formulary of India**, on the same Normal/Abnormal/Unknown basis.
- **`assess_bleeding_risk_labs(cbc_platelets, pt_inr_aptt)`** — flags elevated baseline bleeding risk before starting an anticoagulant. Only an explicit **Abnormal** result (or a numeric PT/INR/aPTT value past threshold) raises risk; **Unknown** never does.
...
`tests/` is a `unittest` suite of **72 tests** covering every tier: `test_clinical.py` (renal/hepatic/bleeding-risk/age-weight assessment from objective Normal/Abnormal/Unknown lab categories), `test_ddi.py` (drug-drug interaction matching), `test_triage.py` (all three triage states across Clopidogrel, Tacrolimus, and Warfarin — including the lab-driven "mandatory TDM" warnings and the requirement that only an explicit Abnormal result or DDI ever escalates, never Unknown), and `test_rules.py` (the downstream genotype/phenotype dosing engine, including Warfarin's combined CYP2C9 + VKORC1 categorization).
